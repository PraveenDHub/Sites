...
window.onload = function () {
  ...
  const selectDistict=document.getElementById("distict");
  selectDistict.disabled = true;

  //Add state option new
  data.states.forEach((value)=>{
    selectState.appendChild(createoption(value.state,value.state));
  });

      //Add distict with respect to state
  selectState.addEventListener("change",(e)=>{
      selectDistict.innerHTML="";
      selectDistict.append(createoption("Select Distict",""));
      selectDistict.disabled=false;
      console.log(e.target.value);
      data.states.forEach(detail =>{
          if(e.target.value==detail.state){
            let newdistict=detail.districts;
            newdistict.forEach(distict =>{
              selectDistict.appendChild(createoption(distict,distict))
            });
          }
      })
});
...
function createoption(displayValue,valueName){
  const newoption=document.createElement("option");
  newoption.value=valueName;
  newoption.text=displayValue;
  return newoption;
}